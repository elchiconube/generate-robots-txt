...
const RobotCss = ({ isLoading = false }: RobotProps) => {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      if (!leftEyeRef.current || !rightEyeRef.current || !robotRef.current)
        return;
      ...
      const calculateEyePosition = (eyeCenter: { x: number; y: number }) => {
        ...
        const distance = Math.min(
          5,
          Math.hypot(e.clientX - eyeCenter.x, e.clientY - eyeCenter.y) / 12
        );
  ...
  return (
    <div className={styles.container}>
      ...
      >
        ...
        <div
          className={`${styles.ears} ${isLoading ? styles.loadingEars : ""}`}
        >
        ...
        <div
          className={`${styles.mouth} ${isLoading ? styles.loadingMouth : ""}`}
        >
          <div
            className={`${styles.tooth} ${
              isLoading ? styles.loadingTooth : ""
            }`}
          ></div>
          <div
            className={`${styles.tooth} ${
